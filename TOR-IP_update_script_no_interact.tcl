...
proc prompt_quiet {cli errorInfo} {
   if [catch {cli_exec $cli "enable"} result] {
      error $result $errorInfo
   }
   if [catch {cli_exec $cli "conf terminal"} result] {
      error $result $errorInfo
   }
   if [catch {cli_exec $cli "file prompt quiet"} result] {
      error $result $errorInfo
   }
   if [catch {cli_exec $cli "end "} result] {
      error $result $errorInfo
   }
}

proc prompt_noisy {cli errorInfo} {
   if [catch {cli_exec $cli "enable"} result] {
      error $result $errorInfo
   }
   if [catch {cli_exec $cli "conf terminal"} result] {
   ...
   if [catch {cli_exec $cli "file prompt noisy"} result] {
      error $result $errorInfo
   }
   if [catch {cli_exec $cli "end "} result] {
      error $result $errorInfo
...
proc delete_file {file_path syslog_msg cli errorInfo} {
	if [file exists $file_path] {
		...
		if [catch {cli_exec $cli "delete /force $file_path"} result] {
			error $result $errorInfo
		}
	}
}
...
prompt_quiet $cli(fd) $errorInfo
if [catch {cli_exec $cli(fd) "copy scp://username:password@ipaddress/ipblacklist.txt flash:ipblacklist_updt.txt"} result] {
...
if [expr $ipblacklist_updt_file_size != $ipblacklist_file_size && $ipblacklist_updt_file_size > 0] {
	...
	prompt_quiet $cli(fd) $errorInfo	
	if [catch {cli_exec $cli(fd) "rename flash:ipblacklist_updt.txt flash:ipblacklist.txt"} result] {
		error $result $errorInfo
	}
} else {
